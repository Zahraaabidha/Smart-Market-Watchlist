...
export function AttentionRow({
  ...
  return (
    ...
    >
      ...
      <span className="w-16 shrink-0 font-semibold text-ink-900 sm:w-20">
        {change.symbol}
      </span>
      <span className="hidden w-32 shrink-0 truncate text-[11px] text-ink-400 lg:block xl:w-40">
        {company ?? ""}
      </span>
      <span
        className={`w-16 shrink-0 text-right font-semibold tabular-nums ${up ? "text-up" : "text-down"}`}
      >
      ...
      <span className="w-2.5 shrink-0 text-right text-ink-300">›</span>
    </button>
  );
}
